...
static FORCEINLINE T Lerp(const T& lhs, const T& rhs, U t) { return lhs * (1.0 - t) + rhs * t; }
...
inline static uint RoundUpToPowerOfTwo(uint Arg)
{
...
{
	return (int)v & static_cast<int>(0x80000000);
}

inline static float Xor(float a, int v)
{
...
static FORCEINLINE uint16_t FloatToHalf(float _f) 
{
...
static FORCEINLINE float HalfToFloat(uint16_t _h) 
{
	const union {
		uint32_t u;
		float f;
